...
public class ConnectionPoint {
    ...
    public void connect(ConnectionPoint point) {
        if (point != null && point != this && !connectedPoints.contains(point)) {
            // Check logic component input restriction on both ends
            if (owner.isLogicComponent()) {
                int connectionIndex = owner.getConnectionPointIndex(this);
                if (owner.isInputConnection(connectionIndex) && !connectedPoints.isEmpty()) {
                    // Logic input already has a connection, don't allow another
                    return;
                }
            }
            if (point.getOwner().isLogicComponent()) {
                int pointConnectionIndex = point.getOwner().getConnectionPointIndex(point);
                if (point.getOwner().isInputConnection(pointConnectionIndex) && !point.getConnectedPoints().isEmpty()) {
                    // Logic input on other side already has a connection, don't allow another
                    return;
                }
            }
